...
# restart replicas #
ansible -i ${OUTPUT_DIR}/db_hosts.txt -m shell -a "sudo service mariadb restart" dbreplica01 -u $ansible_user --private-key=$priv_key --become -o > ${OUTPUT_DIR}/setup_replication_dbreplica01_restart.txt
ansible -i ${OUTPUT_DIR}/db_hosts.txt -m shell -a "sudo service mariadb restart" dbreplica02 -u $ansible_user --private-key=$priv_key --become -o > ${OUTPUT_DIR}/setup_replication_dbreplica02_restart.txt
